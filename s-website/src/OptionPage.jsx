  ...
  const getClassroom = async (roomnumber) => {
    if (roomnumber.length == 0) {
      ...
      try {
        ...
        const userClass = pb.authStore.model.classroom.replace(
          /[&\/\\#,+()$~%.'":*?<>{}]/g,
          ""
        );

        if (
          userClass == capitalize(roomPath) ||
    ...
    <>
      ...
      <center className="pt-10 lg:pt-14">
        ...
        <button
          ...
          className="mt-10 lg:mt-12 bg-[#F95959] outline-[#F95959] outline text-white text-sm lg:text-base font-bold rounded-lg py-2 lg:py-3 px-24 lg:px-48"
        >
