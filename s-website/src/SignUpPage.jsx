...
export default function SignUp() {
    ...
    const signUp = async (
        ...
        //Email Check
        (!emailRegex.test(Email) || Email == null || Email == undefined) ? setInvalidEmailAddress(true) : setInvalidEmailAddress(false);
        // First Name Check
        (!nameRegex.test(FirstName) || FirstName == null || FirstName == undefined) ? setInvalidFirstName(true) : setInvalidFirstName(false);
        // Middle Name Check
        (!nameRegex.test(MiddleName)) ? setInvalidMiddleName(true) : setInvalidMiddleName(false);
        if (MiddleName == null || MiddleName == undefined || MiddleName == "") setInvalidMiddleName(false);
        // Last Name Check
        (!nameRegex.test(LastName) || LastName == null || LastName == undefined) ? setInvalidLastName(true) : setInvalidLastName(false);
        // Password Check
        (Password.length < 5 || Password == null || Password == undefined) ? setInvalidPassword(true) : setInvalidPassword(false);
        // Confirm Password Check
        (PasswordConfirm.length < 5 || PasswordConfirm == null || PasswordConfirm == undefined || PasswordConfirm == " " || PasswordConfirm != Password) ? setInvalidConfirmPassword(true) : setInvalidConfirmPassword(false);

        // If all fields are valid, create account
        if (!(invalidFirstName && invalidMiddleName && invalidLastName && invalidPassword && invalidConfirmPassword)) {
            try {
                const data = {
                            ...
                            invalidConfirmPassword ? (
                                <>
                                    ...
                                    <div className="text-red-500 mt-3">
                                        ...
                                        - Field must be at least 5 characters long
                                        <br />
                                        - Field must be the same as the one above
                                    </div>
                                </>
                            ) : (
                                <>
